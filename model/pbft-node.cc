...
// 全局变量 是所有节点间共用的
int tx_size = 10;
int tx_speed = 100;                  // 交易生成的速率，单位为op/s
int n;
int v;
int val;
float timeout;                   // 发送区块的间隔时间
int n_round;
int testingMessage = 10; // tinggal ganti nilai variabel messagenya untuk pengujian
			  //
        ...
        if (InetSocketAddress::IsMatchingType (from))
        {
            ...
            switch (charToInt(msg[0]))
            {
                ...
                {   
                    ...
                    data[1] = msg[1];           // v = 1
                    data[2] = msg[2];           // n = 0
                    data[3] = msg[3];           // value harusnya nilai n juga
                    // 序号值
                ...
                {   
                    ...
                    tx[index].commit_vote++;
                     //NS_LOG_INFO("nilai commit_vote : "<< tx[index].commit_vote);
                    ...
                    if (block_num == 1){
                    // Send(data, from);
                    NS_LOG_INFO("KONSENSUS SELESAI DALAM WAKTU : " << Simulator::Now ().GetSeconds () << " second");
...
{   
      NS_LOG_INFO ("================### FUNGSI SEND BLOCK (SendBlock) ###================");
  ...
  if (n_round == 1) {
    NS_LOG_INFO(" Telah terkirim dengan jumlah  : "<< n_round << " block dalam waktu: " << Simulator::Now ().GetSeconds () << "s");
