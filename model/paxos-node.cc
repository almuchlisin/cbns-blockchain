...
PaxosNode::StartApplication ()              // Called at time specified by Start
{
   ...
   for (int i = 0; i < N; i++) {
    ...
    if (!m_socket)
    {
        ...
        //NS_LOG_INFO("masuk dan terbaca if tidak m_socket1: " << m_socket);
        // 注意 相当于监听所有网卡ip
    ...
    while(iter != m_peersAddresses.end()) {
        ...
        //NS_LOG_INFO("nilai m_socketClient: " << m_socketClient);
        socketClient->Connect (InetSocketAddress(*iter, 7071));
        m_peersSockets[*iter] = socketClient;
                //NS_LOG_INFO("nilai m_peersSocket dari Client: " << m_peersSockets[*iter]);
    ...
    */
    ...
    if (m_id == 0 || m_id == 1 || m_id == 2) {
        NS_LOG_INFO("nilai m_id yang diproses: " << m_id);
...
{	
    ...
    while ((packet = socket->RecvFrom (from)))
    {
        ...
        {
            std::string msg = getPacketContent(packet, from); //getPacketContent returnnya string
            //NS_LOG_INFO (" paket size " << packet->GetSize () << " isi paket : " << packet);
            packetSizeRecord = packet->GetSize();
            //NS_LOG_INFO (" msg size " << sizeof (msg) << " isi msg : " << msg);
            ...
            // 打印接收到的结果
            NS_LOG_INFO("Node " << GetNode ()->GetId ()); // << " isi pesan / Total Received Data: " << msg);
            ...
            switch (charToInt(msg[0]))
            {
                ...
                {   
                    NS_LOG_INFO ("================### FUNGSI REQUEST TICKET ###================");

                    data[0] = intToChar(RESPONSE_TICKET);
                    int t = charToInt(msg[1]);
                    //NS_LOG_INFO ("nilai msg request ticket : " << msg <<", jumlahnya : " << sizeof(msg) );                      
                    ...
                    if (t > t_max) {
                        ...
                        data[2] = command;              // ['type', 'success', 'command']
                            ...
                            // for (uint8_t i = 3; i < packetSizeRecord; i++){
                       ...
                       // NS_LOG_INFO ("nilai data di request ticket : " << data <<", jumlahnya : " << sizeof(data) );
                        //NS_LOG_INFO ("nilai msg request ticket : " << msg <<", jumlahnya : " << sizeof(msg) );                      
                ...
                {
                    NS_LOG_INFO ("================### FUNGSI REQUEST PROPOSE ###================");
                ...
                case REQUEST_COMMIT:
                {
                    NS_LOG_INFO ("================### FUNGSI REQUEST COMMIT ###================");
                ...
                {
                    NS_LOG_INFO ("================### FUNGSI RESPONSE TICKET ###================");
                    ...
                    NS_LOG_INFO ("nilai msg di response ticket : " << msg);
                    ...
                    if (vote_success + vote_failed == N-2) {
                        if (vote_success >= N / 2) {
                            ...
                            NS_LOG_INFO ("nilai proposal = msg 2 : " << msg);
                            ...
                            NS_LOG_INFO ("nilai data di response ticket : " << data <<", jumlahnya : " << sizeof(data) );
                ...
                {
                    NS_LOG_INFO ("================### FUNGSI RESPONSE PROPOSE ###================");
                ...
                {
                    NS_LOG_INFO ("================### FUNGSI RESPONSE COMMIT ###================");
                ...
                case CLIENT_PROPOSE:
                {
                    NS_LOG_INFO ("================### FUNGSI CLIENT PROPOSE ###================");
...
{ 
    ...
    std::string totalReceivedData(totalStream.str());
       // NS_LOG_INFO("hasi akhir getPacketContent: " << totalReceivedData);
...
// 向所有邻居节点广播消息
void 
PaxosNode::Send (uint8_t data[]) //coba ubah k string, mau kebaca ga ptr nya
{	
    Ptr<Packet> p;
    //uint8_t data[] = "hello";

    p = Create<Packet> (data, testingMessage);
    //NS_LOG_INFO("packet size nya di fungsi send: " << p->GetSize ());
    //if (!m_peersSockets) {
    TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");
    //m_socketClient = Socket::CreateSocket (GetNode (), tid);
    //NS_LOG_INFO("m_socketClient: " << m_socketClient);
...
PaxosNode::requireTicket(void) {
    ...
    //     //NS_LOG_INFO("print data for send: "<< data);
    // }
    //data[2] = intToChar(178491);
    //NS_LOG_INFO("require_data yang mau dikirim ke send: "<< data);
    Send(data);
