...
int tx_size = 10;                   // the size of tx, in KB
int tx_speed = 100;                 // the rate of transaction generation, in op/s
int max_blocks = 10;                 // Maximum number of blocks to process
...
PoaNode::StopApplication ()
{
    ...
    // Log tambahan untuk menunjukkan durasi simulasi per node
    std::cout << "Node " << m_id << " simulation completed in " << duration.GetSeconds() << " seconds." << std::endl;
}
...
// Main program to initialize the simulation and capture total time
int main (int argc, char *argv[])
{
    ns3::Time::SetResolution (ns3::Time::NS);
    ns3::LogComponentEnable ("PoaNode", ns3::LOG_LEVEL_INFO);

    // Catat waktu simulasi dimulai
    ns3::Time startTime = ns3::Simulator::Now();

    // Setup node, aplikasi, dll.
    // Buat jaringan PoA Node sesuai kebutuhan (misalnya 10 atau 100 node)
    
    // Jalankan simulasi
    ns3::Simulator::Run();

    // Catat waktu simulasi selesai
    ns3::Time endTime = ns3::Simulator::Now();
    ns3::Time duration = endTime - startTime;

    // Tampilkan durasi simulasi secara global
    std::cout << "Total simulation time: " << duration.GetSeconds() << " seconds." << std::endl;
